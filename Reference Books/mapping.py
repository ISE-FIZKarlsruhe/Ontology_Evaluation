...
with open('callister_terms.txt', 'r', encoding='utf-8') as file:
    terms1_lines = file.readlines()

with open('callister_original.txt', 'r', encoding='utf-8') as file:
    terms2_lines = file.readlines()

with open('callister_index_chapter_mappings.txt', 'r', encoding='utf-8') as file:
...
with open('terms_mapped_to_chapter.txt', 'w') as file:
    file.write('\n'.join(sorted_terms))
...
for term in sorted_terms:
    ...
    if len(parts) == 2:
        ...
        term_name = term
        chapter = 0  # If chapter is not provided, assign to chapter 0
...
plt.savefig('frequency_of_terms_by_chapter_callister.png')  # Save the plot
plt.show()